...
离线:
  python app/emby_traffic_verify.py
  python app/emby_traffic_verify.py --offline

在线（需 Web 服务运行，默认 http://127.0.0.1:8765）:
  python app/emby_traffic_verify.py --online --instance "MyEmby" --seconds 60
  python app/emby_traffic_verify.py --online --url http://127.0.0.1:8765
...
一键自动验算:
  python app/emby_traffic_verify.py --auto -u <user> -p <pass>
...
def _web_port_from_local_config() -> int | None:
    ...
    workspace_cfg = _ROOT / 'data' / 'config.yaml'
...
def run_offline():
    from app.emby_traffic_verify_offline import run_all
    run_all()
...
def _print_auto_report(
    *,
    ...
    if router_wan_delta_mb is not None:
        ...
        print('\n【对比路由】未提供路由读数。')
        print('  播放前: python app/emby_traffic_verify.py --auto ... --router-wan-start-mb <读数>')
